...
const Rating = ({ rating }) => (
  <div className="flex-center space-x-1">
    {rating > 2 && <img src={star} className="w-3 h-3" alt="Star" />}
    <span className="font-light text-xs">{rating > 2 && rating}</span>
  </div>
);
...
const RenderHouseImages = ({ hoveredItems, item }) => {
  return (
    <>
      <img
        className="rounded-xl flex-center w-full h-full object-cover scroll-snap-align-start"
        src={item.images[0]}
...
const HouseCard = ({
  ...
  return (
    ...
    >
      <motion.div
        className="w-full relative flex flex-col gap-y-2 h-full"
      ...
      >
        ...
        <div
          ...
          className="w-full flex items-center justify-start overflow-x-auto hide-scrollbar scroll-smooth relative"
          style={{
